...
class ResultSet4Test(iterator: Iterator[Map[Int, Any]], metadata: Map[String, Int] = null) extends ResultSet{
  private var values: Map[Int, Any] = new Map[Int, Any] {
    override def +[B1 >: Any](kv: (Int, B1)): Map[Int, B1] = ???

    override def get(key: Int): Option[Any] = ???

    override def iterator: Iterator[(Int, Any)] = ???

    override def -(key: Int): Map[Int, Any] = ???

    override def apply(key: Int) = throw new SQLException("Before start of result set")
  }
  ...
  override def close(): Unit = {
    println("result set has be closed")
  }

  override def wasNull(): Boolean = ???

  override def getString(columnIndex: Int): String = try {
    values(columnIndex).asInstanceOf[String]
  } catch {
    case e: ClassCastException => throw new Exception(s"${values(columnIndex)}", e)
    case e: Throwable => throw e
  }

  override def getBoolean(columnIndex: Int): Boolean = values(columnIndex).asInstanceOf[Boolean]

  override def getByte(columnIndex: Int): Byte = values(columnIndex).asInstanceOf[Byte]

  override def getShort(columnIndex: Int): Short = ???

  override def getInt(columnIndex: Int): Int = values(columnIndex).toString.toDouble.toInt

  override def getLong(columnIndex: Int): Long = ???

  override def getFloat(columnIndex: Int): Float = ???

  override def getDouble(columnIndex: Int): Double = values(columnIndex).toString.toDouble
  ...
  override def getObject(columnIndex: Int): AnyRef = values(columnIndex).asInstanceOf[AnyRef]
